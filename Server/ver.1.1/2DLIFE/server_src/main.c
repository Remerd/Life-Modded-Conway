...
int get_color(int ul, int um, int ur, int ml, int mr, int bl, int bm, int br, int mmc,int ulc, int umc, int urc, int mlc, int mrc, int blc, int bmc, int brc)
{
int i,sum_c;
int color[5]={0};
for (i=0;i<5;i++)
 ...
 for(i=0;i<N;i++)
    {
     for(j=0;j<N;j++)
		...
		b[i][j][1] = get_color(a[i-1][j-1][0],a[i-1][j][0],a[i-1][j+1][0], a[i][j-1][0],a[i][j+1][0], a[i+1][j-1][0],a[i+1][j][0],a[i+1][j+1][0],a[i][j][1],a[i-1][j-1][1],a[i-1][j][1],a[i-1][j+1][1], a[i][j-1][1],a[i][j+1][1], a[i+1][j-1][1],a[i+1][j][1],a[i+1][j+1][1]);
