...
$(function() {
	...
	ws.onmessage = function(e) {
		...
		var idd =a0+'_'+a1+'_'+a2;			
		if (a3 == 9 && blanks) {
			$('#'+idd).remove();
		} else if (a0 < limit && a1 < limit && a2 < limit) {
			if ($('#'+idd).length) {				
				if ($('#'+idd).attr('material').color != colors[a3]) {
					if (colors[a3]) {
						$('#'+idd)[0].setAttribute('material', {color: colors[a3], metalness: 0, roughness: 10});
					} else {
				...
				if (colors[a3]) {
					entity.setAttribute('material', {color: colors[a3], metalness: 0, roughness: 10});
				} else {
